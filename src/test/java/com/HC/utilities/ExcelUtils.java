...
public class ExcelUtils {
	 ...
	 * iTestCaseStart) throws Exception{ for(int
	 * i=iTestCaseStart;i<=ExcelUtils.getRowCount(SheetName);i++){
	 * if(!sTestCaseID.equals(ExcelUtils.getCellData(i, Constants.Col_TestCaseID,
	 * SheetName))){ int number = i; return number; } } ExcelWSheet =
	 * ExcelWBook.getSheet(SheetName); int number=ExcelWSheet.getLastRowNum()+1;
	 * return number; }
	 */
    	}